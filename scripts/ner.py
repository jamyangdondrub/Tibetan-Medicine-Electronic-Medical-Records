...
def main():
    ...
    parser.add_argument("--max_seq_length", default=256, type=int)#原本256
    ...
    parser.add_argument("--clean", default=False,
                        type=boolean_string, help="clean the output dir")
    ...
    writer = SummaryWriter(logdir=os.path.join(
        args.output_dir, "eval"), comment="Linear")
    ...
    if args.do_train:
        ...
        train_examples, train_features, train_data = get_Dataset(
            args,processor, tokenizer, mode="train")
        train_sampler = RandomSampler(train_data)
        train_dataloader = DataLoader(
            train_data, sampler=train_sampler, batch_size=args.train_batch_size)
        if args.do_eval:
            eval_examples, eval_features, eval_data = get_Dataset(
                args, processor, tokenizer, mode="eval")
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                        ...
                        writer.add_scalar(
                            "Train/loss", tr_loss_avg, global_step)
                        logging_loss = tr_loss

            if args.do_eval:
                all_ori_tokens_eval = [f.ori_tokens for f in eval_features]
                overall, by_type = evaluate(
                    args, eval_data, model, id2label, all_ori_tokens_eval)
    ...
    if args.do_test:
        ...
        test_examples, test_features, test_data = get_Dataset(
            args, processor, tokenizer, mode="test")
