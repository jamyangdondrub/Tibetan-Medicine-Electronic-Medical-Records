...
import sentencepiece as spm # 后加
import  regex as re #  后加
...
class NerProcessor(object):
    ...
    def get_labels(self, args):
        ...
        if os.path.exists(os.path.join(args.output_dir, "label_list.pkl")):
            ...
            logger.info(
                f"loading labels info from train file and dump in {args.output_dir}")
            ...
            if len(labels) > 0:
                ...
                logger.info(
                    "loading error and return the default labels B,I,O")
                labels = {"O", "B", "I"}
...
def convert_examples_to_features(args, examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in tqdm(enumerate(examples), desc="convert examples"):
        ...
        for i, word in enumerate(textlist):
            ...
            # yang= r'[\u0F00-\u0FFF]+|.'
            # token = re.findall(yang,word)
            token = re.findall(r'\X+', word)
        ...
        # sp.load('bert-tibetan/pytorch_model.bin')
        ...
        #     else:
        #         #print(f"Token '{token}' 不在 sentencepiece 词汇表中,替换为UNK")
        #          input_ids.append(UNK_ID)

        input_mask = [1] * len(input_ids)

        assert len(ori_tokens) == len(
            ntokens), f"{len(ori_tokens)}, {len(ntokens)}, {ori_tokens}"
